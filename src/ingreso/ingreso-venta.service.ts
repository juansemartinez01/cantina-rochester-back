...
export class IngresoVentaService {
  ...
  async registrarIngreso(dto: CreateIngresoVentaDto) {
    ...
    const detallePago = this.validarDetallePago(dto.tipo, dto.detalle_pago);
  ...
  async obtenerTodosConFiltros(
    filtros: FiltroIngresoVentaDto & {
      page?: number;
      limit?: number;
      ordenCampo?: string;
    ...
    if (fechaDesde)
      query.andWhere('ingreso.fecha >= :fechaDesde', {
        fechaDesde: new Date(fechaDesde),
      });
    if (fechaHasta)
      query.andWhere('ingreso.fecha <= :fechaHasta', {
  ...
  async obtenerResumen(filtros: {
    ...
    return {
      ...
      otro: totalOtro,
      total: totalEfectivo + totalBancarizado + totalOtro,
    };
  }

  private validarDetallePago(
    tipo: CreateIngresoVentaDto['tipo'],
    value: string | undefined,
  ): string | null {
    const detallePago = normalizarDetallePago(value);
    if (!requiereDetallePago(tipo)) return null;

    if (!detallePago || detallePago.length < 3) {
      throw new BadRequestException(
        'detalle_pago es obligatorio para ingresos con tipo OTRO',
      );
    }
